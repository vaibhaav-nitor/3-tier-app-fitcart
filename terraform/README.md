...
group, a VNet, an Azure Container Registry, a Key Vault, a managed PostgreSQL
server, and an AKS cluster joined to the VNet subnet.

```
terraform/
├── bootstrap/        one-time, local state — creates the remote state backend
├── modules/          resource-group, networking, acr, key-vault, postgresql, aks
...
| Resource | Name | Notes |
|---|---|---|
| Virtual network | `vnet-fitcart-dev` | `10.50.0.0/16`; `snet-aks` is `10.50.1.0/24` |
...
| PostgreSQL server | `psql-fitcart-dev-<suffix>` | Flexible Server, `B_Standard_B1ms`, public access + firewall rule |
...
|---|---|---|
| Create resources in the group | `Contributor` | VNet, ACR, Key Vault, PostgreSQL, AKS, storage |
...
Charts live in [../helm](../helm), one release per tier — frontend and backend
only. The database is provisioned by Terraform, not Helm; `terraform apply` has
to run before the first backend deploy, since the backend needs `POSTGRES_HOST`
to exist. After that:

1. **Backend — Build & Deploy**
2. **Frontend — Build & Deploy**

Each workflow then triggers independently on pushes touching its own paths.

The Postgres password is generated by Terraform, stored only in Key Vault and
state, and read back by the deploy workflow at run time. It is never a GitHub
secret and never committed. The same credentials are the managed database's
actual administrator login — nothing is duplicated between the two.
...
Left running, the footprint costs roughly **$110–130/month** — the single
`Standard_D2s_v3` node at about $70, plus the managed database
(`B_Standard_B1ms`, 32Gi storage, 7 days backup) at roughly $15–20.

Two constraints shaped the node choice, both specific to this subscription:
...
One node is adequate here: with the database now running outside the cluster,
the two remaining tiers request about 250m CPU and 576Mi total, against roughly
1.9 vCPU and 5.5Gi allocatable — comfortably more headroom than when Postgres
ran in-cluster too. The trade-off is still no surge capacity during cluster
upgrades and no resilience to node loss. Raise `node_count` once quota allows —
the AKS module deliberately does *not* set `ignore_changes` on it, so the
change takes effect.
